...
class OwlEgoController:
    ...
    def __init__(self, image_dir=None, config_path=None, *, hardware=None, config=None):
        ...
        self.hw = hardware
        from ..owl_ego.queries import OwlQueries
        self.queries = OwlQueries(hardware, self.config)
    ...
    def _mutate(self, path, data):
        ...
        if path == '/v22/land':
            return self._command('land',session_id=sid,task_id=tid)
    ...
    def observation(self, *, include_image=True):
        ...
        try:
            ...
            if isinstance(exc, ValueError) and not hasattr(exc, 'error_code'):
                error = InvalidObservation(str(exc))
    ...
    def health(self):
        ...
        try:
            self.observation(include_image=False)
            h['rgb_ok'] = True
        except Exception as e:
            h['rgb_ok'] = False
