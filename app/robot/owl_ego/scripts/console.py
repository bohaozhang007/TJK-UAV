...
import time
import uuid
from console_client import Runner, Failure, parse_args
...
class Console:
    ...
    def command(self,line):
        ...
        if cmd=='help':
            print('init | takeoff | move_rel_xyz_yaw X Y Z YAW | move_rel_xyz X Y Z | land | stop | wait | health | pose | quit',flush=True)
...
def main():
    ...
    print('启动仅连接客户端；init、takeoff 完成并停稳后，在另一终端启动 v22 Agent。',flush=True)
    try:
        if args.commands:
