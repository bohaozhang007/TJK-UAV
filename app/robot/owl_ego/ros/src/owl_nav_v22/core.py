...
def validate_terminal_stop(trajectory, goal):
    end, velocity, _ = trajectory.sample(trajectory.start + trajectory.duration)
    if np.linalg.norm(end - np.asarray(goal[:3])) <= .05 and np.linalg.norm(velocity) > .02:
        raise Rejected('goal trajectory must have zero terminal velocity')
