"""Public mission interfaces. Positions: cm, yaw: degrees clockwise, world Y right."""
from dataclasses import dataclass
from typing import Protocol


class ControlLost(RuntimeError):
    pass


class MissionError(RuntimeError):
    pass


class TargetNotLocalizable(MissionError):
    """This detection has insufficient geometric support."""


class NavigationPlanningFailed(MissionError):
    """Planning failed before execution; the previous task is cleared and stopped."""


@dataclass
class Observation:
    frame_id: str
    pose: dict
    epoch: str
    rgb: object
    image_base64: str
    intrinsics: object
    world_from_camera_cm: object
    timestamp_s: float


class Robot(Protocol):
    def attach(self): ...
    def health(self): ...
    def pose(self): ...
    def wait_stopped(self): ...
    def observe(self, timings=None) -> Observation: ...
    def autofocus_photo(self, observation, box, timings=None): ...
    def preview_path(self, goal, require_arrival=False, timings=None): ...
    def locate_target(self, observation, mask, timings=None): ...
    def point_is_free(self, pose, timings=None): ...
    def points_are_free(self, poses, timings=None): ...
    def navigate(self, pose, timings=None): ...
    def stop(self): ...
    def land(self): ...
    def close(self): ...
