...
## 2026-09-10 补充：Console交互历史和方向键

发送方Robot；回复用户VS Code终端方向键无法调用历史命令的问题。原console直接
使用Python input()，未加载readline行编辑支持；命令进入console后不由bash读取历史。
现仅在交互输入分支import readline，启用上下键历史及左右键编辑；模块不可用时给出
提示并继续普通输入。命令文件执行和飞行控制逻辑不变，历史仅保留于当前console进程。
现场Python的GNU readline可用。实际console伪终端验证：输入help、上键重用、下键
返回新输入、左右键及Delete纠错均得到预期命令，正常quit；日志确认没有HTTP请求。
未操作真实服务/飞机。落地结束会话后重开console加载，无需重启bridge/server；Agent
接口无变化。
