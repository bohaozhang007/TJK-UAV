# v15：新增边界安全的多阶段 best-view 排名和重检测失败回退。
...
def _configure_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("tjk_v15")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "YAML path relative to src/agent/config, such as owl/v15.yaml; "
            "absolute paths are also supported. Defaults to owl/v15.yaml "
            "for the OWL v15 agent"
        ),
    )
    parser.add_argument(
        "--exp_name",
    ...
    parser.add_argument(
        ...
        default="owl",
        help=(
            "Robot to use; v15 is configured for OWL (default: owl)"
        ),
    )
    parser.add_argument(
        "--server-host",
...
def _resolve_config_path(config_path: str | None, client_name: str) -> Path:
    ...
    if config_path is None:
        return (config_root / "owl" / "v15.yaml").resolve()
