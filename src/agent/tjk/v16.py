# v16：新增 SAM3 文本/视觉示例检测，保留 SAM2 作为跟踪器。
...
def _configure_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("tjk_v16")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "YAML path relative to src/agent/config, such as owl/v16.yaml; "
            "absolute paths are also supported. Defaults to owl/v16.yaml "
            "for the OWL v16 agent"
        ),
    )
    parser.add_argument(
        "--exp_name",
    ...
    parser.add_argument(
        ...
        default="owl",
        help=(
            "Robot to use; v16 is configured for OWL (default: owl)"
        ),
    )
    parser.add_argument(
        "--server-host",
...
def _resolve_config_path(config_path: str | None, client_name: str) -> Path:
    ...
    if config_path is None:
        return (config_root / "owl" / "v16.yaml").resolve()
