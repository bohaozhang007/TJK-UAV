# v20：SEARCH 每圈以首个 view 的 XYZ 为锚点，后续旋转均发送 XYZ+Yaw 对齐命令。
...
def _config_bool(section: dict, key: str) -> bool:
    if key not in section:
        raise ValueError(f"Missing config value: {key}")
    value = section[key]
...
def _configure_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("tjk_v20")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "YAML path relative to src/agent/config, such as i7/v20.yaml; "
            "absolute paths are also supported. Defaults to i7/v20.yaml "
            "for the I7 v20 agent"
        ),
    )
    parser.add_argument(
        "--exp_name",
...
def _resolve_config_path(config_path: str | None, client_name: str) -> Path:
    ...
    if config_path is None:
        if client_name == "i7":
            return (config_root / "i7" / "v20.yaml").resolve()
