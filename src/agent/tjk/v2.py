...
class TJKAgent:
    def __init__(
        ...
        self.gd_box_threshold = 0.35
        self.gd_text_threshold = 0.25

    def set_vis_dir(self, vis_dir):
        self.vis_dir = vis_dir
        self.tracker.set_vis_dir(vis_dir)
    ...
    def init_tracker(self, text_prompt):
        ...
        for i in range(12):
            ...
            if detection is not None:
                ...
                show_fig(cur_frame, f"{self.vis_dir}/candidate_{i}.png", box_coords=target_box)
                print(f"[SEARCH] Found {label} with confidence={confidence:.3f} at heading={i * 30} deg.")
                break

            show_fig(cur_frame, f"{self.vis_dir}/candidate_{i}.png")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--vis_dir", type=str, default='./tjk_vis')
    parser.add_argument("--text_prompt", type=str, default='white ball')
    parser.add_argument(
        "--client",
        choices=("telloClient", "ueClient"),
        default="telloClient",
        help="Robot Client implementation to use (default: telloClient)",
    )
    return parser
...
def main():
    ...
    print(f"[MODE] GroundingDINO search: {args.text_prompt!r}")
    ...
    tjkAgent = TJKAgent(client=client, vis_dir=args.vis_dir)
    tjkAgent.connect()
    tjkAgent.run(args.text_prompt)
